...
export default function DiscountSlider() {
  ...
  const settings = {
    ...
    customPaging: () => (
      <div className="w-3 h-3 bg-gray-300 rounded-full hover:bg-yellow-400 transition-all duration-300 cursor-pointer"></div>
    ),
  };
  ...
  return (
    <div className="relative w-full h-[450px] md:h-[600px] overflow-hidden">
      ...
      <Slider {...settings} className="h-full">
        {offers.map((offer) => {
          const brandPath = offer.brand ? `/brands/${(offer.brand || '').toLowerCase().replace(/\s+/g, '-')}` : '/products';
          const productsHref = offer.brand
            ? `/products?brand=${encodeURIComponent(offer.brand)}`
            : `/products?category=${encodeURIComponent(offer.category || '')}`;
          ...
          return (
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') router.push(brandPath); }}
              className="relative w-full h-[450px] md:h-[600px] block cursor-pointer"
            >
              {/* Background Image */}
              <Image
                src={offer.image}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-6 z-20">
                <div className="max-w-4xl mx-auto">
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); router.push(productsHref); }}
                    onKeyDown={(e) => { if (e.key === 'Enter') { e.stopPropagation(); router.push(productsHref); } }}
                    className="inline-block px-8 py-4 bg-gradient-to-r from-yellow-400 to-orange-400 hover:from-yellow-500 hover:to-orange-500 rounded-full 
